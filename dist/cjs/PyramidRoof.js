...
exports.PyramidRoof = void 0;
const maplibre_gl_1 = require("maplibre-gl");
const maplibre_gl_style_spec_1 = require("@maplibre/maplibre-gl-style-spec");
// extend the Point type to include a z property
class Point3D extends maplibre_gl_1.Point {
    constructor(x, y, z) {
        super(x, y);
        this.z = z;
        this.z = z || 0;
...
class PyramidRoof {
    constructor(params) {
        ...
        if ((0, maplibre_gl_style_spec_1.isExpression)(params.base)) {
            let baseExpression = (0, maplibre_gl_style_spec_1.createExpression)(params.base);
            ...
            this.base = typeof params.base === 'string' || Array.isArray(params.base) ? (0, maplibre_gl_style_spec_1.createExpression)(params.base) : Number(params.base);
        }
        if ((0, maplibre_gl_style_spec_1.isExpression)(params.height)) {
            let heightExpression = (0, maplibre_gl_style_spec_1.createExpression)(params.height);
            ...
            this.height = typeof params.height === 'string' || Array.isArray(params.height) ? (0, maplibre_gl_style_spec_1.createExpression)(params.height) : Number(params.height);
        }
    }
    ...
    parseHeight(height) {
        try {
            let heightExpression = new maplibre_gl_style_spec_1.StyleExpression(height);
    ...
    getVerticesFromCoordinates(coords, height, base) {
        ...
        let mercatorCenter = maplibre_gl_1.MercatorCoordinate.fromLngLat({ 'lng': center[0], 'lat': center[1] }, center[2]);
        ...
        coords.forEach(coord => {
            ...
            let mercatorCood = maplibre_gl_1.MercatorCoordinate.fromLngLat({ lng: coord[0], lat: coord[1] }, zValue);
    ...
    getVerticesFromRings(ring, height, base) {
        try {
            ...
            // Add the center point first
            let mercatorCenter = maplibre_gl_1.MercatorCoordinate.fromLngLat({ 'lng': center[0], 'lat': center[1] }, height // Use scaled height
            );
            if (!this.debug_logged) {
            ...
            for (let point of ring) {
                let mercatorPoint = maplibre_gl_1.MercatorCoordinate.fromLngLat({ 'lng': point.x, 'lat': point.y }, base // Use scaled base
                );
    ...
    getMLVerticesFromRings(ring, height, base) {
        ...
        */
        ...
        for (let p = 0; p < ring.length; p++) {
            ...
            let c1 = maplibre_gl_1.MercatorCoordinate.fromLngLat({ 'lng': p1.x, 'lat': p1.y }, p1.z || base);
            p1 = new Point3D(c1.x, c1.y, c1.z);
            //}
            if (p >= 1) {
                let p2 = ring[p - 1];
                let c2 = maplibre_gl_1.MercatorCoordinate.fromLngLat({ 'lng': p2.x, 'lat': p2.y }, p2.z || base);
    ...
    getVerticesForFeature(feature) {
        try {
            ...
            if (this.height instanceof maplibre_gl_style_spec_1.StyleExpression) {
            ...
            if (this.base instanceof maplibre_gl_style_spec_1.StyleExpression) {
...
exports.PyramidRoof = PyramidRoof;
exports.default = PyramidRoof;
